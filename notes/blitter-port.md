...
and their six-digit counters) into `scr_buff` before `buff_to_vga`
using the original HUD sprite block (`spr_1up`, `spr_2up`, `spr_hi`,
`spr_score_digits`). The visual-test executable is compiled with
`BATTY_SCORELESS_HUD` because the GT capture pipeline intentionally
NOPs the original score block.
