...
Spectrum and is not a defect in the port. Numbers are stable references — other
notes and gate scripts cite them by number — so they are never reused, and the
gaps in the sequence are numbers nothing cites.

## Status

| # | what | state |
|---|------|-------|
| #1 | stale-VGA leftovers after brick destruction | fixed; `test-enemy-brick-residue`, `notes/performance.md` |
| #2 | inverted-colour residue after an enemy fly-over | fixed; `test-enemy-brick-residue`, `notes/performance.md` |
| #3 | multi-hit bricks shimmer continuously | fixed; `test-shimmer-one-pass`, `notes/metal-shimmer.md` |
| #6 | the ball tunnels through a row-0 metal brick | fixed; `test-ball-no-tunnel`, `notes/laffc-decode.md` |
| #7 | moving sprites recoloured the attribute cells they flew over | fixed; `test-sprite-attr-parity`, `notes/video-engine.md` |
...
| #19 | ENTER steered bat 2, and ENTER starts every capture | fixed; ENTER dropped from the bat-2 cluster, `test-double-play-input` |
...
The sections below keep only what the fix does not say for itself.
...
extras with `dir_to_dxdy` like the primary.
...
(`bounce_wall`) belongs to the ball and the sparks.
...
Every gate asserting a derived value was audited for the same shape.
...
then CAPTURED row 31 into the static cache — which made the line permanent
rather than a flicker. `band_rebuild_window` in `src/bricks.cpp` owns the
paint window and the capture window as one decision.
...
`invalidate_static_cache_after_death()` asks for the repaint explicitly.

It hid for months because losing a life changes the life counter, and
`refresh_static_background` rebuilds on `lives_dirty` for the indicators'
sake — the repaint the explosion needed was arriving free from a counter it
has nothing to do with. `BATTY_INFINITE_LIVES` removed the coincidence.
...
arithmetic instead. Closing the hole means giving the test build a HUD and
re-baselining every visual gate. Not done.
