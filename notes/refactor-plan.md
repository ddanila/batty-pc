...
**The code.** `main.cpp`: 7,747 → 6,944 lines (-10.4%) across 15
...
**The tests.** 78 gates, indexed in `notes/testing.md` and kept complete
by `test-gate-index`. They fall in three groups:

  - 61 QEMU gates — `make parity-check-parallel`, ~6 min, twelve clean
    runs, the latest covering the `handle_input` split.
  - 17 emulator-free source gates plus 14 host suites — `make test-fast`,
...
`main.cpp`: 7,747 → 6,944 (`wc -l`; see the status block on why this is not Watcom's count).
