...
**The code.** `main.cpp`: 7,747 → 7,466 lines (-3.6%) across 15
...
**The tests.** 83 gates, indexed in `notes/testing.md` and kept complete
by `test-gate-index`. They fall in three groups:

  - 63 QEMU gates — `make parity-check-parallel`, ~6 min, twelve clean
    runs, the latest covering the `handle_input` split.
  - 20 emulator-free source gates plus 14 host suites — `make test-fast`,
...
`main.cpp`: 7,747 → 7,466 (`wc -l`; see the status block on why this is not Watcom's count).
