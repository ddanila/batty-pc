...
**The code.** `main.cpp`: 7,747 → 8,354 lines (net -1) across 15
...
**The tests.** 105 gates, indexed in `notes/testing.md` and kept complete
by `test-gate-index`. They fall in three groups:

  - 76 QEMU gates — **`python3 scripts/run_gates_parallel.py --full`**,
...
`main.cpp`: 7,747 → 7,907 (`wc -l`; see the status block on why this is not Watcom's count).
