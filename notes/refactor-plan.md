...
**The code.** `main.cpp`: 7,747 → 7,735 lines (-0.2%) across 15
...
**The tests.** 86 gates, indexed in `notes/testing.md` and kept complete
by `test-gate-index`. They fall in three groups:
  ...
  - 22 emulator-free source gates plus 14 host suites — `make test-fast`,
...
`main.cpp`: 7,747 → 7,735 (`wc -l`; see the status block on why this is not Watcom's count).
