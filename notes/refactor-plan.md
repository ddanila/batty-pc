...
**The code.** `main.cpp`: 7,747 → 7,846 lines (net -1) across 15
...
**The tests.** 94 gates, indexed in `notes/testing.md` and kept complete
by `test-gate-index`. They fall in three groups:

  - 66 QEMU gates — `make parity-check-parallel`, ~6 min, twelve clean
...
`main.cpp`: 7,747 → 7,846 (`wc -l`; see the status block on why this is not Watcom's count).
