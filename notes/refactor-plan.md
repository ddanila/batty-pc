...
**The code.** `main.cpp`: 7,747 → 7,234 lines (-6.6%) across 15
...
**The tests.** 82 gates, indexed in `notes/testing.md` and kept complete
by `test-gate-index`. They fall in three groups:

  - 62 QEMU gates — `make parity-check-parallel`, ~6 min, twelve clean
...
`main.cpp`: 7,747 → 7,234 (`wc -l`; see the status block on why this is not Watcom's count).
