 ...
 *
 * There are TWO "is this brick there" rules in this codebase and they are
...
/* Reset destroyed cells to the band background across char rows [cr0, cr1]
 * — the rows the caller just re-based from level_attrs.
 *
 ...
 * mattered when the base band was a capture taken with every brick ALIVE.)
 * What earns its keep is the SHADOW half: a destroyed cell's left char goes
 * non-bright when its left neighbour is still live, and nothing else writes
 * that.
 *
 * The row scan runs one brick row beyond [r0, r1] on each side, because cr0
 * doubles as row r0-1's shadow row and cr1 as row r1+1's cell row. That
 * overlap is what known-bugs #1 and #2 were. */
void reset_destroyed_cell_attrs(const u8 *cells, u8 bg_attr,
                                int r0, int r1, int cr0, int cr1);

/* The whole brick band from a level's captured attrs. The caller adds the
 * border shadow, which is a frame concern rather than a brick one. */
void paint_brick_band(const u8 *cells, const u8 *lattr, u8 bg_attr);

/* The same, scoped to brick rows [r0, r1] and the char rows [cr0, cr1] they
 * re-base — the incremental rebuild's path. Unlike the full paint this must
 * repair its own edges, because a repaint that stops at a boundary does not
 * get the next row's overwrite for free. */
void paint_brick_band_rows(const u8 *cells, const u8 *lattr, u8 bg_attr,
                           int r0, int r1, int cr0, int cr1);

/* Bricks only, rows [first_row, last_row]. Callers must repaint or
 * otherwise account for the neighbouring rows' edges. */
 ...
 * back what the row beyond the range would have written. Getting this wrong
 * is what known-bugs #1 and #2 were. */

/* `row`'s live bricks own the first pixel row of their body, which the row
 * above's bottom edge would otherwise have zeroed. */
 ...
 * re-composited, which pixel rows the caller must repaint from background
 * first, and which pixel rows it then captures into its static cache.
 *
 * The two pixel windows are NOT the same. `py_capture0` is the top-edge row
 * SHARED between brick row r0 and the row above it: repaint_row_top_edge
 * writes it from the row below, so the rebuild has to capture it. Whether
 * it may also be ERASED first depends on whether a row above exists:
 *
 *   r0 > 0   the row above owns that pixel row's content and is not being
 *            re-composited. Erasing it would drop the neighbour's bottom
 *            edge, so repainting starts one row LOWER and print re-zeros it
 *            under r0's live bricks.
 *   r0 == 0  there is no row above. The zeros there belong to row 0's own
 *            bricks, so when one of those is destroyed nobody rewrites the
 *            row — it has to be erased with the rest.
 *
 * Missing the r0 == 0 case leaves a solid black line where the top brick
 * row used to be, baked into the static cache and therefore permanent.
 * known-bugs.md #20. */
