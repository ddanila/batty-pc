#!/usr/bin/env python3
"""Compare enemy-bomb dirty redraw against a forced full redraw baseline.

Verifies the bomb tier added to render_simple_objects_to_buff_and_mark (so a
falling bomb no longer forces a full-dynamic frame). Bakes a falling bomb
clear of the bat (BATTY_REPLAY_BOMB), probes mid-fall (f10), and asserts the
dirty-path screen equals the BATTY_FORCE_BALL_FULL_REDRAW baseline.
"""
...
OUT = Path("build/test_bomb_dirty_redraw")
...
def build_floppy(force_full: bool) -> None:
    ...
    env.update({
        ...
        "BATTY_REPLAY_BOMB": "40,40",
        "BATTY_SUPPRESS_NO_BALL_DEATH": "1",
        "BATTY_VISUAL_PROBE_FRAMES": "10",
    })
    if force_full:
...
def main() -> int:
    ...
    if diff != 0:
        make_diff_png(dirty, full, OUT / "bomb_dirty_redraw_diff.png")
        raise SystemExit(
            f"FAIL: bomb dirty redraw differs from full redraw: "
            f"{diff} px [roi {PLAYFIELD_ROI}]"
        )
    print(f"PASS bomb_dirty_redraw: dirty redraw matches full redraw [roi {PLAYFIELD_ROI}]")
    return 0
