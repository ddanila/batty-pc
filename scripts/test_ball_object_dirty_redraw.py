#!/usr/bin/env python3
"""Compare ball+enemy dirty redraw against a forced full redraw baseline."""
...
OUT = Path("build/test_ball_object_dirty_redraw")
...
def capture(label: str) -> bytes:
    ...
    ppm = out / "ball_enemy_after_frames.ppm"
    script = [
        "SLEEP 9.0",
        "sendkey ret",
        "SLEEP 1.2",
...
def main() -> int:
    ...
    if diff != 0:
        make_diff_png(dirty, full, OUT / "ball_object_dirty_redraw_diff.png")
        raise SystemExit(
            f"FAIL: ball+object dirty redraw differs from full redraw: "
            f"{diff} px [roi {PLAYFIELD_ROI}]"
        )
    print(f"PASS ball_object_dirty_redraw: dirty redraw matches full redraw [roi {PLAYFIELD_ROI}]")
    return 0
