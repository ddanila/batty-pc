#!/usr/bin/env python3
"""Compare stuck-ball dirty redraw against a full redraw baseline.

A stuck (pre-launch / MAGNET-held) ball rides the bat at a known position
and is visible, so it no longer forces a full-dynamic frame — it redraws on
the dirty path like a moving ball. Bakes a stuck ball (BATTY_REPLAY_BALL_STUCK
=1) and asserts the dirty path is pixel-identical to the
BATTY_FORCE_BALL_FULL_REDRAW baseline a few frames in (bat static, ball
riding it).
"""
...
OUT = Path("build/test_stuck_ball_dirty_redraw")
...
def build_floppy(force_full: bool) -> None:
    ...
    env.update({
        ...
        "BATTY_REPLAY_BALL_STUCK": "1",      # ball rides the bat (pre-launch)
        "BATTY_VISUAL_PROBE_FRAMES": "5",
    })
    if force_full:
...
def capture(label: str) -> bytes:
    ...
    ppm = out / "stuck_ball_after_frames.ppm"
    script = [
        "SLEEP 9.0",
        "sendkey ret",
        "SLEEP 1.2",
...
def main() -> int:
    ...
    if diff != 0:
        make_diff_png(dirty, full, OUT / "stuck_ball_dirty_redraw_diff.png")
        raise SystemExit(
            f"FAIL: stuck-ball dirty redraw differs from full redraw: "
            f"{diff} px [roi {PLAYFIELD_ROI}]"
        )
    print(f"PASS stuck_ball_dirty_redraw: dirty redraw matches full redraw [roi {PLAYFIELD_ROI}]")
    return 0
