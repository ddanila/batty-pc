#!/usr/bin/env python3
"""Compare ball-only dirty redraw against a forced full redraw baseline."""
...
OUT = Path("build/test_ball_dirty_redraw")
...
def main() -> int:
    ...
    if diff != 0:
        make_diff_png(dirty, full, OUT / "ball_dirty_redraw_diff.png")
        raise SystemExit(
            f"FAIL: ball-only dirty redraw differs from full redraw: "
            f"{diff} px [roi {PLAYFIELD_ROI}]"
        )
    print(f"PASS ball_dirty_redraw: dirty redraw matches full redraw [roi {PLAYFIELD_ROI}]")
    return 0
